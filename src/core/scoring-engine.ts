...
function aspirationFit(career: Career, profile: StudentProfile): number {
  ...
  if (!goal) return 0.5;
  if (goal === "higher_study") return career.higherStudyRequired === "mandatory" ? 0.9 : 0.6;
  if (goal === "job_soon") return career.minYearsToEarn && career.minYearsToEarn <= 4 ? 0.9 : 0.4;
  if (goal === "business") return career.riskLevel === "entrepreneurial" ? 0.9 : 0.5;
  if (goal === "government") return 0.6;
  return 0.5;
}
